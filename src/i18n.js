    ...
    translation: {
      ...
      privacy: {
        ...
        fullHtml: `
          ...
          <p>Nachrichtentexte, Absenderadresse und Anhänge, die Sie über das Kontaktformular senden, werden verarbeitet, um Ihre Anfrage zu beantworten. Wir nutzen für den Versand von E‑Mails einen SMTP‑Dienst (konfigurierbar über Umgebungsvariablen). Anbieter: <strong>IONOS</strong> (ersetzen Sie dies nur, falls ein anderer Anbieter verwendet wird). Kontaktmails werden standardmäßig für <strong>6 Monate</strong> gespeichert und anschließend gelöscht, sofern keine gesetzlichen Aufbewahrungsfristen entgegenstehen. In Entwicklungsumgebungen können Nachrichten vorübergehend in <code>server/mail-dev.log</code> geschrieben werden; auf Produktivsystemen sollte dies vermieden werden.</p>
    ...
    translation: {
      ...
      privacy: {
        ...
        fullHtml: `
          ...
          <p>Contact form messages, sender addresses and attachments are processed to respond to enquiries. We use an SMTP service for delivery (configured via environment variables). Provider: <strong>IONOS</strong> (replace this only if using a different provider). Contact messages are retained for <strong>6 months</strong> by default and then deleted unless legal retention obligations apply. In development, messages may be written to <code>server/mail-dev.log</code>; avoid persistent storage of sensitive content on production systems.</p>
